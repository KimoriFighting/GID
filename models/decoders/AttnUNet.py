...
class AttU_Net(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels, n_classes):
        ...
        self.Att3 = Attention_block(F_g=128, F_l=64, F_int=64)
        self.Up_conv3 = conv_block(ch_in=192, ch_out=128)
        ...
        # 额外添加
        # self.conv1 = nn.Conv2d(512, 1024, kernel_size=3, stride=2, padding=1, bias=False)
        # self.bn1 = nn.BatchNorm2d(1024)
        # self.relu = nn.ReLU(inplace=True)
        #
        # self.conv2 = nn.Conv2d(256, 1024, kernel_size=5, stride=4, padding=2, bias=False)
...
if __name__ == "__main__":
    ...
    model = AttU_Net(output_ch=9)

    y, fea, fea2, fea3 = model(x)
    print(y.shape)
    print(fea.shape)
    print(fea2.shape)
    print(fea3.shape)
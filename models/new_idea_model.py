...
class GAN_Uet(nn.Module):
    def __init__(self, num_classes=9, vt_type='ViT-L_16', img_size=224, middle=[3, 6, 9]):
        ...
        self.decoder = UNet_idea([config.hidden_size*2, 512*2, 256*2, 64*2], [512*2, 256*2, 64*2, 16*2], [512*2, 256*2, 64*2, 0], num_classes)
    ...
    def forward(self, x):
        ...
        y, image = self.decoder(feature, mid_tmp)
...
if __name__ == "__main__":
    batch_size, num_classes, h, w = 1, 9, 256, 256
    x = torch.autograd.Variable(torch.randn(batch_size, 3, h, w))

    model = GAN_Uet(vt_type='ViT-L_16', img_size=256)
    y, mid_y = model(x)
    print(y.shape)
    print("..........")
    for i in mid_y:

...
def main(args, rnd):
    ...
    model = LCLT_Net(vt_type='Vit-L').to(device)
    # teacher = TUNet2(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size, middle=[9, 14, 19]).to(device)
    teacher = TUNet(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size,
                     middle=[9, 14, 19]).to(device)
    # teacher = 1
    #
...
def test(args, rnd):
    ...
    # state = torch.load('./save/GID_R50_Unet0.1_{}'.format(
    #                rnd), map_location=device)
    state = torch.load('/mnt/disk/yongsen/code/Semantic_Segmentation/LCLT/Student', map_location=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre_path', type=str,
                        default='/search/hadoop04/jiangxinfa/yongsen/GID/save/transformerL_Unet_199.pth')

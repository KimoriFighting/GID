...
def main(args):
    ...
    model = GameNet(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size, middle=[9, 14, 19]).to(device)
    ...
    optimizer_G = optim.SGD(model.parameters(), lr=args.base_lr*3, momentum=0.9, weight_decay=0.0001)
    optimizer_D = optim.SGD(model.parameters(), lr=args.base_lr, momentum=0.9, weight_decay=0.0001)
    # optimizer_D = torch.optim.Adam(discriminator.parameters(), args.base_lr, betas=(0.5, 0.999))
    scheduler_G = LR_Scheduler_Head(args.lr_scheduler, args.base_lr*3,
                                  args.epochs, len(trainloader), warmup_epochs=10)
    scheduler_D = LR_Scheduler_Head(args.lr_scheduler, args.base_lr,
                                  args.epochs, len(trainloader), warmup_epochs=10)

    for epoch in range(args.epochs):
        train_new_idea(trainloader, model, discriminator, ce_loss, dice_loss,
              optimizer_G, optimizer_D, device, scheduler_G, scheduler_D, epoch)
    ...
    torch.save(save_files,
               "/mnt/disk/yongsen/code/Semantic_Segmentation/LCLT/Teacher/transformer_L_game_{}.pth".format(
                   epoch))

def test(args):
    device = torch.device(args.cuda if torch.cuda.is_available() else "cpu")
    # model = Student(num_classes=args.num_classes).to(device)
    model = GameNet(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size, middle=[9, 14, 19]).to(
        device)
    state = torch.load("/mnt/disk/yongsen/code/Semantic_Segmentation/LCLT/Teacher/transformer_L_game_{}.pth".format(
                   args.epochs-1), map_location=device)
    ...
    db_test = HemangiomaDataset_val(r'/mnt/disk/yongsen/Semantic_Segmentation/hemangioma/img',
                                    opt=transforms.Compose([
                                        transforms.Resize((args.img_size, args.img_size)),
                                        transforms.ToTensor(),
                                        transforms.Normalize([0.485, 0.456, 0.406],
    ...
    for sample in testloader:
        ...
        Image.fromarray(((out > 0.5) * 255).astype(np.uint8)).save(os.path.join(r'/mnt/disk/yongsen/Semantic_Segmentation/hemangioma/trainpredict', sample['name'][0]))

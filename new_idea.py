...
def main(args):
    ...
    testloader = DataLoader(db_test, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True,
                             worker_init_fn=worker_init_fn)
    model = GAN_Uet(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size, middle=[9, 14, 19]).to(device)

    d2d = Discriminator(3, args.img_size, args.img_size).to(device)

    ce_loss = CrossEntropyLoss()
    dice_loss = DiceLoss(args.num_classes)
    optimizer = optim.SGD(model.parameters(), lr=args.base_lr, momentum=0.9, weight_decay=0.0001)
    optimizer_D = torch.optim.Adam(d2d.parameters(), lr=0.0002, betas=(0.5, 0.999))
    scheduler = LR_Scheduler_Head(args.lr_scheduler, args.base_lr,
                                  args.epochs, len(trainloader), warmup_epochs=5)
    scheduler_D = LR_Scheduler_Head(args.lr_scheduler, 0.0002,
                                  args.epochs, len(trainloader), warmup_epochs=5)

    for epoch in range(args.epochs):
        train_new_idea(trainloader, testloader, model, d2d, ce_loss, dice_loss, optimizer, optimizer_D, device, scheduler,
                       scheduler_D, epoch)
...
def test(args):
    ...
    model = GAN_Uet(num_classes=args.num_classes, vt_type=args.vit_name, img_size=args.img_size, middle=[9, 14, 19]).to(device)
    state = torch.load('/home/yongsen/Segments/new_idea/resnet152_299.pth', map_location=device)

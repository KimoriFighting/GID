...
def main(args):
    ...
    teacher = Teacher(vt_type=args.vit_name, middle=[2, 5, 8, 11]).to(device)
    ...
    for epoch in range(args.epochs):
        loss = train_stu(trainloader, model, teacher, ce_loss, dice_loss, kl_loss, optimizer, device, scheduler, epoch, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vit_name', type=str,
                        default='ViT-B_16', help='select one vit model', choices=['ViT-B_16, ViT-L_16'])
    parser.add_argument('--batch_size', type=int, default=8, help='batch_size per gpu')
    parser.add_argument('--n_skip', type=int,
                        default=4, help='using number of skip-connect, default is num')
    parser.add_argument('--save_name', type=str,
                        default='lclt1111_transformerS_unet')
